...
const refreshIcon = document.querySelector("#rotateCtn");
...
//creating a function that takes an array of shows and renders the shows
function display(array) {
  ...
  array.forEach((item) => {
    ...
    <p> <i class="fa-solid fa-circle"></i>Status: ${item.status}</p>\n<p> <i class="fa-solid fa-film"></i>Genres: ${item.genres}</p>\n<p><i class="fa-solid fa-hourglass-start"></i>Runtime: ${item.runtime}</p>`;
...
showSelect.addEventListener("change", () => {
  ...
  allShows.forEach((x) => {
    if (selectValue === x.name) {
      ...
      display(newArray);
      episodeSelect.style.display = "none";
      renderDisplayNum(displayNumEpi, newArray, allShows);
    }
  });
});
...
searchbar.addEventListener("input", (e) => {
  let result = allShows.filter((x) => {
    return (
      ...
      x.summary.toLowerCase().includes(e.target.value.toLowerCase())
    );
  });
  display(result);
...
/// defining the function that gets episodes with an api
...
//.........displaying episodes for each show pages for episode

function makePageForEpisodes(array) {
  renderDisplayNum(displayNumEpi, array, array);
  rootElem.innerHTML = "";
  wrapper.innerHTML = "";
  array.forEach((x) => {
